...
function MapPinIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5 shrink-0" aria-hidden="true">
...
function ChevronDown({ open }: { open: boolean }) {
  return (
    <svg className={`h-3 w-3 fill-none stroke-current transition-transform duration-200 ${open ? "rotate-180" : ""}`} viewBox="0 0 24 24" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    ...
    <div className="relative" ref={ref}>
      ...
      >
        <span className="text-primary-foreground/60 transition-colors duration-200 group-hover:text-primary-foreground shrink-0">{icon}</span>
                    <span className={`flex-1 font-body text-body outline-none truncate ${selected ? "font-medium text-primary-foreground" : "text-primary-foreground/65"}`}>
      ...
      {open && (
        <div
          role="listbox"
          className="absolute left-0 right-0 z-30 mt-1 rounded-lg border border-border bg-surface/95 backdrop-blur-md py-1 shadow-lg max-h-24 overflow-y-auto"
        >
          {options.map((opt) => (
            <button
              key={opt.value}
